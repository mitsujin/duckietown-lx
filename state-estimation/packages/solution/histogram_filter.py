...
def kinematic_model(left_encoder_ticks, right_encoder_ticks, robot_spec):
    alpha = 2 * np.pi / robot_spec['encoder_resolution']
    rotation_wheel_left = left_encoder_ticks * alpha 
    rotation_wheel_right = right_encoder_ticks * alpha 
    d_left = rotation_wheel_left * robot_spec['wheel_radius']
    d_right = rotation_wheel_right * robot_spec['wheel_radius']
    d_A = (d_left + d_right) / 2.0 
    theta = (d_right - d_left) / robot_spec['wheel_baseline']
    return (d_A, theta)
...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    v,w = kinematic_model(left_encoder_ticks, right_encoder_ticks, robot_spec)
